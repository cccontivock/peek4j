...
public final class AgentFrameworkLauncher {
	public static final String STARTUP_STDERR_CANARY = AgentFrameworkLauncher.class.getName() + " started.";
	...
	public static void launchAgentFramework(String agentArgsStr, Instrumentation instrumentation) {
		Log.logOut(AgentFrameworkLauncher.class, STARTUP_STDERR_CANARY);
		...
		try (InputStreamReader propsReader = new InputStreamReader(
			...
			Log.logErr(AgentFrameworkLauncher.class, "Erred while processing properties file.");
			ex.printStackTrace();
		}
		...
		Log.logOut(AgentFrameworkLauncher.class, "Peek4J Agent HTTP server will use port " + httpPortStr);
...
//		Log.logOut(AgentFrameworkLauncher.class, "Peek4J Agent will use log4j configuration " + log4jCfgUrlStr);
		try {
			final Launcher bndFwLauncher = new Launcher(properties, null);
			bndFwLauncher.activate();
			Runtime.getRuntime().addShutdownHook(new Thread(() -> {
				try {
					Log.logOut(AgentFrameworkLauncher.class, "Peek4J Agent deactivating OSGi framework.");
			...
			Log.logOut(AgentFrameworkLauncher.class, "Peek4J Agent launched OSGi framework.");
			...
			Log.logOut(AgentFrameworkLauncher.class, "Peek4J Agent using args: " + agentArgs);
			...
			Log.logOut(AgentFrameworkLauncher.class, "Peek4J Agent registered Instrumentation instance as a service.");
		} catch (final Exception ex) {
			Log.logErr(AgentFrameworkLauncher.class, "Erred while launching OSGi framework.");
			ex.printStackTrace();
		} finally {
